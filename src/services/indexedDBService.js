...
class AnswerAIDB extends Dexie {
    constructor() {
        ...
        this.version(1).stores({
            // Files table: stores uploaded documents
            ...
            // Settings table: stores app settings (will migrate from localStorage)
            settings: 'key'
        })
    }
}
...
export async function deleteFileFromIndexedDB(fileId) {
    try {
        ...
        console.log(`🗑️ File and chunks deleted from IndexedDB: ${fileId}`)
    } catch (error) {
...
export async function clearAllFilesFromIndexedDB() {
    try {
        ...
        console.log('🗑️ All files and chunks cleared from IndexedDB')
    } catch (error) {
